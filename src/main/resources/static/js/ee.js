...
var RTCMultiConnection = function(roomid, forceOptions) {
                ...
                return {
                    ...
                    emit: function(eventName, data, callback) {
                        ...
                        if (eventName === 'open-room' || eventName === 'join-room') {
                            colsole.log("-- call that.io.emit("+eventName+").callback(true,"+data.sessionid+",null)");
    ...
    function SocketConnection(connection, connectCallback) {
        ...
        try {
            ...
            console.log("-- connection.socket=io("+connection.socketURL + parameters+")");
        } catch (e) {
            connection.socket = io.connect(connection.socketURL + parameters, connection.socketOptions);
            console.log("socket:e="+connection.socket+" "+connection.socketURL + parameters);
        }
...
console.log("-- onMessageEvent():0: message.sender="+message.sender);
...
console.log("-- onMessageEvent(): message.message.streamSyncNeeded="+message.message.streamSyncNeeded);
...
console.log("-- onMessageEvent(): connection.deletePeer()");
...
console.log("-- onMessageEvent(): participant="+participant);
            ...
            if (message.message.newParticipant) {
                ...
                console.log("-- onMessageEvent(): call mPeer.createNewPeer()");
...
console.log("-- onMessageEvent(): connection.waitingForLocalMedia="+connection.waitingForLocalMedia);
                    setTimeout(function() {
...
console.log("-- onMessageEvent(): message.message.newParticipationRequest="+message.message.newParticipationRequest+", message.sender="+message.sender+", connection.userid="+connection.userid);
...
console.log("-- onMessageEvent(): onNewParticipant(message.sender= "+message.sender+")");
...
console.log("-- onMessageEvent(): delete connection.peers[",message.message.oldUUID,"]");
                    delete connection.peers[message.message.oldUUID];
                }
            }

            if (message.message.userLeft) {
console.log("-- onMessageEvent(): mPeer.onUserLeft(",message.sender,")");
...
console.log("-- onMessageEvent(): mPeer.addNegotiatedMessage()");
        ...
        connection.socket.on('disconnect', function(event) {
            console.log(".socket.on('disconnect'):0:event.userid="+event.userid);
        ...
        connection.socket.on('user-disconnected', function(remoteUserId) {
            ...
            console.log(".socket.on('user-disconnected'):0:remoteUserId="+remoteUserId);
            ...
            console.log(".socket.on('user-disconnected'):1");
        });

        connection.socket.on('user-connected', function(userid) {
            console.log(".socket.on('user-connected'):0:userid="+userid);
            ...
            console.log(".socket.on('user-connected'):1");
        });
    ...
    function MultiPeers(connection) {
        ...
        this.addNegotiatedMessage = function(message, remoteUserId) {
            console.log("-- addNegotiatedMessage():", message.type?"message.type="+message.type:"",  message.sender?"message.sender="+message.sender:"");
    ...
    function PeerInitiator(config) {
        ...
        this.addRemoteCandidate = function(remoteCandidate) {
            console.log("-- addRemoteCandidate(): remoteCandidate="+remoteCandidate);
...
console.log("-- addRemoteSdp():",peer.iceConnectionState?"iceConnectionState="+peer.iceConnectionState:"",peer.iceGatheringState?"iceGatheringState="+peer.iceGatheringState:"",peer.signalingState?"signalingState="+peer.signalingState:"");
            if (DetectRTC.browser.name !== 'Safari') {
                remoteSdp.sdp = connection.processSdp(remoteSdp.sdp);
            }
            ...
            peer.setRemoteDescription(new RTCSessionDescription(remoteSdp)).then(cb, function(error) {
                if (!!connection.enableLogs) {
                    console.error('setRemoteDescription failed:11', '\n', error, '\n', remoteSdp.sdp);
                }

                cb();
            }).catch(function(error) {
                if (!!connection.enableLogs) {
                    console.error('setRemoteDescription failed:12', '\n', error, '\n', remoteSdp.sdp);
                }
        ...
        if (config.remoteSdp) {
            ...
            this.addRemoteSdp(config.remoteSdp, function() {
                console.log("-- addRemoteSdp()'s callback: call createOfferOrAnswer('createOffer')");
        ...
        function createOfferOrAnswer(_method) {
            console.log("-- createOfferOrAnswer(): _method="+_method);
        ...
        if (isOfferer) {
            console.log("-- PeerInitiator(): call createOfferOrAnswer('createOffer')");
            ...
            if (options.localMediaConstraints.isScreen === true) {
                if (navigator.mediaDevices.getDisplayMedia) {
                    navigator.mediaDevices.getDisplayMedia(options.localMediaConstraints).then(function(stream) {
                        ...
                        options.onLocalMediaError(error, options.localMediaConstraints);
                        currentUserMediaRequest.mutex = false;	// 2022/09/22 added by DKLEE <- in case of cancel button
                    });
...
console.log("-- onRemovingRemoteMedia()(): delete connection.streamEvents[",stream.streamid,"]");
        ...
        connection.openOrJoin = function(roomid, callback) {
            callback = callback || function() {};
console.log("-- openOrJoin(): connection.checkPresence()");
            connection.checkPresence(roomid, function(isRoomExist, roomid) {
                if (isRoomExist) {
                    ...
                    beforeJoin(connectionDescription.message, function() {
                        console.log("-- openOrJoin():beforeJoin: call joinRoom()");
                ...
                if (isData(connection.session)) {
                    console.log("-- openOrJoin(): call openRoom():<-isData");
                    openRoom(callback);
                    return;
                }

                connection.captureUserMedia(function() {
                    console.log("-- openOrJoin(): call openRoom("+roomid+"):<-captureUserMedia");
                    openRoom(callback);
                });
            });
        };
        ...
        connection.deletePeer = function(remoteUserId) {
            ...
            if (!!connection.peers[remoteUserId]) {
                ...
                if (peer && peer.iceConnectionState !== 'closed') {
                    try {
console.log("-- deletePeer(): connection.peers[",remoteUserId,"].peer.close()");
                        peer.close();
                    } catch (e) {}
                }
...
console.log("-- deletePeer(): delete connection.peers[",remoteUserId,"]");
        ...
        connection.join = function(remoteUserId, options) {
            ...
            beforeJoin(connectionDescription.message, function() {
                connectSocket(function() {
                    console.log("-- Join:beforeJoin: call joinRoom():"+JSON.stringify(connectionDescription));
        ...
        function joinRoom(connectionDescription, cb) {
            console.log("-- joinRoom(): socket.emit('join-room')");
 ...
 console.log("-- joinRoom:setTimeout: call joinRoom()");
                        joinRoom(connectionDescription, cb);
                    }, 3000);
                }
        ...
        function openRoom(callback) {
            ...
            connection.waitingForLocalMedia = false;
            console.log("-- openRoom(): socket.emit('open-room')");
...
console.log("== addStream(): "+JSON.stringify(session)+", getTracks="+session.getTracks+", indexOf="+connection.attachStreams.indexOf(session)+", session.streamid="+session.streamid);
...
console.log("== addStream(): 1");
                connection.renegotiate(remoteUserId);
                return;
            }

            if (isData(session)) {
console.log("== addStream(): 2");
                connection.renegotiate(remoteUserId);
                return;
            }
        ...
        connection.invokeGetUserMedia = function(localMediaConstraints, callback, session) {
            ...
            getUserMediaHandler({
                onGettingLocalMedia: function(stream) {
                    ...
                    // 2022/09/20 added by DKLEE : https://github.com/muaz-khan/RTCMultiConnection/issues/863
...
console.log("-- onLocalMediaError:0:error="+error+", constraints="+constraints);
                    mPeer.onLocalMediaError(error, constraints);
console.log("-- onLocalMediaError:1")
                },
        ...
        connection.checkPresence = function(roomid, callback) {
            ...
            console.log("-- checkPresence():0:SocketConnection.name="+SocketConnection.name);	// SocketConnection
            ...
            if (!connection.socket) {
                console.log("-- checkPresence(): call connection.connectSocket()");
                connection.connectSocket(function() {
                   console.log("-- checkPresence(): call connection.checkPresence()");
                    connection.checkPresence(roomid, callback);
                });
                return;
            }

            console.log("-- checkPresence(): call socket.emit('check-presence', roomid="+roomid+")");
